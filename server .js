...
function isCountryMatch(item, targetCountry) {
  ...
  if (targetCountry === 'china') {
    return cStr.includes('trung quốc') || cStr.includes('china') || cStr.includes('trung-quoc');
  } else if (targetCountry === 'japan') {
    if (cStr.includes('trung quốc') || cStr.includes('china') || cStr.includes('trung-quoc')) return false;
    return true;
  }
  return true;
}
...
app.get(['/catalog/:type/:id.json', '/catalog/:type/:id/:extra.json'], async (req, res) => {
  ...
  try {
    ...
    if (id === 'stp_hot' && !selectedGenre) {
      // TỐI ƯU ĐẶC BIỆT CHO TRANG CHỦ (STP_HOT) TRÁNH ĐEN MÀN HÌNH
      const { data } = await axios.get(`https://phimapi.com/danh-sach/phim-moi-cap-nhat?page=${pageToFetch}`, { timeout: 6000 });
      let rawList = data?.items || data?.data?.items || [];
      const metas = rawList.map(item => createCatalogMeta(item, 'movie'));
    ...
    if (isAnimeCatalog) {
      ...
      if (selectedGenre && GENRE_SLUG_MAP[selectedGenre]) {
        ...
        [1, 2, 3, 4, 5].forEach(p => {
          requests.push(axios.get(`https://phimapi.com/v1/api/the-loai/${genreSlug}?page=${p}&limit=30`, { timeout: 5000 }).catch(() => null));
        });
      } else {
        [startP, startP + 1, startP + 2, startP + 3].forEach(p => {
          requests.push(axios.get(`https://phimapi.com/v1/api/danh-sach/hoat-hinh?page=${p}&limit=30`, { timeout: 5000 }).catch(() => null));
        });
      }
      ...
      if (selectedGenre && GENRE_SLUG_MAP[selectedGenre]) {
        ...
        const responses = await Promise.all(
          pagesToFetch.map(p =>
            axios.get(`https://phimapi.com/v1/api/the-loai/${genreSlug}?page=${p}&limit=30`, { timeout: 5000 }).catch(() => null)
          )
        );
        ...
        const { data } = await axios.get(`${apiUrl}?page=${pageToFetch}&limit=30`, { timeout: 6000 });
    ...
    // CHỐNG MÀN HÌNH ĐEN: NẾU BỊ LỌC SẠCH DỮ LIỆU THÌ LẤY LẠI DỮ LIỆU GỐC
    if (metas.length === 0 && !selectedGenre) {
      const { data } = await axios.get(`https://phimapi.com/danh-sach/phim-moi-cap-nhat?page=1`, { timeout: 5000 }).catch(() => ({ data: {} }));
  ...
  if (id.startsWith('phimapi:')) {
    ...
    try {
      ...
      servers.forEach((srv, sIdx) => {
        ...
        if (ep && ep.link_m3u8) {
          streams.push({
            name: `Server FHD [${srv.server_name || `Nguồn 
